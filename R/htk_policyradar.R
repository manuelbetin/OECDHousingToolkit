...
names(vars_needed)
 ...
 # if(any(colnames(data) %in% varcodes)){
  ...
  final<-  final %>% mutate_at(vars(name_vars),as.numeric) %>%
    mutate(value_scaled = ((value.value-value.min) / (value.max-value.min ))*(5-1)+1,
           mean_scaled =  ((value.mean-value.min) / (value.max-value.min )*(5-1)+1 ))

   final_row= data.frame(as_tibble(t(final), rownames = "name_vars"))
   names(final_row) <- final_row[1,]
   final_row <- final_row[-1,]

names(final_row)
final_row$main_v
    data=final_row %>% filter(main_v=="value.max"|main_v=="value.min"|
                              main_v=="value_scaled"|main_v=="mean_scaled")

    #|,
                             # main_v=="value.country_min"|main_v=="value.country_max")


    data=data[c("2","1", "3", "4" ),]
    rownames(data) <- data[,1]
    data <- data[,-1]
    data1<-data.frame(as_tibble(data))
