...
htk_scatterplot2 <- function(data_source,myvar_x,myvar_y,my_label,Xlabel=NULL,Ylabel=NULL,title=NULL,subtitle=NULL)
{
  plot <- ggplot(data =data_source, aes(x=get(myvar_x),y=get(myvar_y),
                                        label=get(my_label))) +
                                        #group=get(my_label))) +
    ...
    scale_x_continuous(name="Peak-to-trough of residential investment", limits=c(-100, 0)) +
    scale_y_continuous(name="Time to recovery of real GDP",limits=c(0, 40)) +
    geom_smooth(method=lm, se = FALSE, aes(x=get(myvar_x), y=get(myvar_y)), color="darkred") +
    labs(title = title,
         subtitle=subtitle,
         x= Xlabel,
         y = Ylabel) +
